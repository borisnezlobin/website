...
<img src="https://boydjohnson.dev/blog/concurrency-matrix-multiplication/matrix-multiplication-good.gif"></img>
...
<img src="https://i.postimg.cc/bJcChf9N/Pasted-image-20240601195718.pngh"></img>
_Credit to 3Blue1Brown._

This can be written as $A\cdot \vec{x}=\vec{v}$. So a system of linear equations is just a vector that, when transformed by $A$, gives $\vec{v}$! The solution to a system of linear equations is the vector $\vec{x}$ that, when transformed by $A$, gives $\vec{v}$.
How do we solve a system of linear equations (SLE)? We can use Gaussian elimination: we write the SLE as an augmented matrix, row reduce it, and then solve the resulting system of equations.<br />
...
**Theorems**<br />
There are a ton of results related to eigenvalues and eigenvectors. Here are a few. Try to prove eaach one, or at least understand why they're true.
...
What I've covered here is just the beginning of linear algebra. There's so much more to learn, and I hope you continue to explore. If you have any questions, feel free to reach out to me at [my email](mailto:me@borisnezlobin.com). Good luck!