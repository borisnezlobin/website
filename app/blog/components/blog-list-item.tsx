...
const BlogListItem = ({
  ...
  <div
    key={post.id}
    className={`${!inGrid ? "mt-8 print:m-0 cursor-pointer group" : "flex flex-shrink-0 relative group cursor-pointer flex-col gap-2 h-48 min-w-64 max-w-[90vw] w-96 p-4 print:p-0 border border-neutral-300 hover:border-neutral-400 dark:border-neutral-600 hover:dark:border-neutral-400 group rounded-lg tranition-all hover:shadow-lg hover:-translate-y-px"}`}
  >
    <Link
      href={`/blog/${post.slug}`}
