[Some practice problems one could perhaps do](https://web.pdx.edu/~erdman/LINALG/Linalg_pdf.pdf)
...
The image of a transformation $T:V\to W$ is denoted $image(T)$, and is the subset of $W$ of all $\left\{ Tv\; |\; v\in V \right\}$. For any linear transformation $T:V\to W$,
1. $\textbf{0}\in image(T)$.
2. $image(T)$ is a subspace of $W$.
If $\dim(V)=\dim(W)$ and $T:V\to W$ is linear, then $T$ is onto if and only if $T$ is one-to-one.
...
$$
\ker(T)=\left\{v\in V \;|\; Tv=0\right\}
$$
For any transformation $T:V\to W$,
1. $\textbf{0}\in \ker(T)$.
2. $\ker(T)$ is a subspace of $V$.
If $V$ is finite dimensional and $T:V\to W$ is linear, then $r(T)+null(T)=\dim(V)$. A transformation is one-to-one if and only if $\ker(T)=\left\{\textbf{0}\right\}$.

**Nullity**
The dimension of the nullspace of $T$ is called the **nullity** of $T$, and is denoted by $null(T)$.

**Injective**
A transformation $T$ is **injective** (one-to-one) if for all vectors $v_1, v_2\in V$, $v_1\neq v_2$ implies $Tv_1\neq Tv_2$.

**Surjective**
A transformation $T$ is **onto** (or surjective) if $W=image(T)$. So $T$ is onto $W$ if every element in the range is in the image of $T$.

**Isomorphic**
If $T:V\to W$ is linear, onto, and one-to-one, then it is **isomorphic**, and an isomorphism exists between $V$ and $W$, denoted $V\cong W$. It is reflective, transitive, and symmetric. Since isomorphism is an **equivalence relation**, $V\cong W$ if and only if $\dim(V)=\dim(W)$ (there exists an isomorphism $T_1:V\to \mathbb{R}^n$ and $T_2:W\to \mathbb{R}^n$). This means that any $n$-dimensional vector space is just a copy of any other $n$-dimensional vector space, and any vector space (up to the names of its elements) is *completely determined* by its dimension.
...
**Squishy Transformations**
If the vectors that $\hat{\textbf{i}}$ and $\hat{\textbf{j}}$ land on are linearly dependent (i.e., the columns of the transformation matrix are linearly dependent), then the transformation transforms all of 2d space into a line.

**Transformation Composition (Matrix Composition)**
...
**Dual Vector**
Transforming to 1 dimension of space is *computationally identical* (see 3Blue1Brown video about duality for intution) to taking the dot product with some vector. For 2d-to-1d transformations,
$$
...
**Multi-dimensional Determinant**
In 3 dimensions, the determinant is the factor by which *volume* is scaled. After that... have fun visualizing! But what is "negative" volume? Use right-hand-rule (you should know this).

**Calculating the Determinant**
Fun stuff, but how do you actually find the determinant?
$$\det\left(\begin{bmatrix}a & b \\c & d\end{bmatrix}\right)=ad-bc$$
But this is only for 2d matrices! What about for $3\times3$ matrices? Hehe.
...
**Linear Systems of Equations**
A system of equations (you know this), where every term is a scalar multiple of the variable — nothing more. Then, each equation models a linear combination of the variables.
...
$$
**Rank**
Some squishes are more "squishy" than others — 3d space into a line is more "squished" than 3d space squished into a plane, but both transformations have determinant $0$. The **rank** describes this — it is the number of dimensions in the output. A transformation whose output is a line has a rank of $1$, while a transformation whose output is a plane has a rank of $2$.
...
If $V$ is a vector space with basis $B$ and $W$ is a basis with basis $C$, and $T:V\to W$ is linear, then there exists a matrix $A_B^C$ such that $(Tv)_C=A_B^C(v)_B$. The $j$th column consists of the coordinate representation of the $j$th vector in $V$'s basis expressed with respect to basis $C$. If $B$ and $C$ are two bases of vector space $V$, then there exists a **transition matrix** $P_B^C$ such that $(v)_C=P_B^C (v)_B$ for all $v\in V$.

If $V$ is a vector space with bases $B$ and $C$ and $T:V\to V$ is linear, then $P_B^CA_B^B=A_C^CP_B^C$.
...
// todo: rewatch that part of lecture 27
...
**Eigenvectors**
Eigenvectors of a transformation are vectors that stay on their span during the transformation. Each eigenvector has an associated...

**Eigenvalue**
An eigenvalue describes the scalar that the eigenvector is multiplied by as a result of the transformation.

Why are eigenvectors important? Consider a 3d rotation — if you can find an eigenvector, you have found the axis of rotation for that transformation.

**Computation**
Computing eigenvectors and eigenvalues will be covered later. For now, note this:
$$
A\vec{v}=\lambda\vec{v},
$$
Where $A$ is a transformation matrix, $\vec{v}$ is an eigenvector, and $\lambda$ is that eigenvector's eigenvalue. This means that the matrix-vector product is the same as simply scaling the eigenvector by the eigenvalue. We can rewrite the above equation to use matrix-vector multiplication ($I$ is the identity matrix) and simplify to obtain the following:
$$
(A-\lambda I)\vec{v}=\vec{\textbf{0}}.
$$
But, the only way we can have a matrix times a nonzero vector multiply to the zero vector is if the determinant is zero. So, we know that
$$
\det\left({A-\lambda I}\right)=0.
$$
For two dimensions, we can solve for $\lambda$, as the determinant gives us a quadratic in $\lambda$ (and we have the quadratic formula!). Then, plug in that/those values of $\lambda$ into the matrix, and solve for which vectors it sends to $\vec{\textbf{0}}$. It's just a linear system!
...
**Quick Trick for Finding Eigenvalues**
...
**What are vectors?** I don't know! They might be "arrows" in "space," or they could be a list of numbers, right?
...
- Addition:
	- (A1) Closure: For all $u,v\in V,$ $u\oplus v\in V$.
	- (A2) Commutativity: For all $u,v\in V$, $u\oplus v=v\oplus u$.
	- (A3) Associativity: For all $u,v,w\in V$, $(u\oplus v)\oplus w=u\oplus (v\oplus w)$.
	- (A4) Existence of an identity: There exists a zero vector in $V$ (denoted $\textbf{0}$) such that for every $v\in V$, $v\oplus \textbf{0}=v$.
	- (A5) Existence of inverse: There exists for every $v\in V$ a $-v$ such that $v\oplus (-v)=0$.
- Multiplication:
	- (S1) Closure: For all $u\in V$ and all scalars $\alpha$, $\alpha\odot u\in V$.
	- (S2) Associativity: For all $u\in V$ and scalars $\alpha, \beta$, $\alpha \odot(\beta\odot u)=(\alpha\cdot\beta)\odot u$.
	- (S3) Non-scaling property: $1$ is an identity element. For all vectors $u\in V$, $1\odot u=u$.
	- (S4) Distributivity (1): For all $u\in V$ and scalars $\alpha$ and $\beta$, $(\alpha + \beta)\odot u=\alpha\odot u \oplus \beta\odot u$. Note that "$+$" refers to standard addition of real and complex numbers.
	- (S5) Distributivity (2): For all $u, v\in V$ and scalars $\alpha$, $\alpha\odot(u\oplus v)=\alpha\odot u \oplus \alpha\odot v$.
...
- For any $v\in V$, if $v\oplus v=v$, $v=\textbf{0}$.
- For any $u\in V$, $u\odot \textbf{0}=\textbf{0}$.
- For any scalar $\alpha$, $\alpha\odot \textbf{0}=\textbf{0}$.
- For any $v\in V$, $(-1)\odot v=-v$.
- For any $v\in V$, $v=-(-v)$.
- For any $v\in V$ and scalar $\alpha$, if $a\odot v=0$, either $a=0$ or $v=\textbf{0}$.

**Spanning Set**
A set $S$ is a spanning set of $V$ if $span(S)=V$. This means that "$S$ spans $V$." If $S\subset V$, then:
1. If $span(S)=V$, then some subset of $S$ is a basis for $V$.
2. If $S$ is linearly independent, then $S$ is a subset of some basis for $V$.
Lemmas:
1. If $V$ is a vector space and $S\subset T\subset V$, then $span(S)\subset span(T)$.
2. If $V$ is a vector space and $S\subset V$, then $span(span(S))=span(S)$.
3. If $V$ is a vector space and $S,T\subset V$ such that $S\subset T$, then $span(S)\subset span(T)$.
4. If $v_1,\dots, v_n$ are vectors in vector space $V$, then
...
**Basis**
A **basis** for $V$ is a *linearly independent* spanning set of $V$. Basically a "minimal" spanning set. A vector space is **finitely dimensional** if it has a basis with finitely many elements.

Thm. If the set $S=\left\{ v_1,v_2,\dots,v_n \right\}$ spans $V$, then any set $T$ with more than $n$ elements must be linearly dependent. From this it follows that if $B=\left\{v_1, v_2,\dots, v_n \right\}$ is a basis for $V$, then every linearly dependent subset of $V$ must have $n$ or fewer elements. Also, every basis for $V$ has the same number of elements. The number of elements in a basis for $V$ is called the **dimension** of $V$.

Note: The solutions to any linear homogenous differential equation of order $n$ is a vector space of dimension $n$.

Bases are used to express any vector in the vector space. For any vector $\vec{u}\in V$, there is only one way to express $\vec{u}$ as a linear combination of the basis vectors. The coefficients used in this linear combination are called the **coordinates**, and can be written as $n$-tuples or column matrices. 
