...
When the posting asks for a skill beyond the core technical one, spend a bullet on it. Writing, presenting, or communicating findings calls for his editing, journalism, research-presentation, or debate bullets; a startup or founding role calls for a founder or client bullet; money, markets, budgets, or business ownership calls for the bullets about running a budget, sponsorships, or cutting costs; teaching or mentoring calls for the teaching bullets. One such bullet is usually enough.
Do not pick two bullets that carry the same fact; if two would repeat a claim, keep the one that states it better and use the slot for something new.
Prefer depth over breadth: two bullets from one strong project beat single bullets from two weaker ones, and at most five projects reach the page.
